...
if a==1
    x=(2/pi)*(((pi/2)+b*V).*tan(V)-b*log((W.*cos(V))./((pi/2)+b*V))); % page 489, isnt a pi/2 missing before W in the log?i
    x=c*x+d-(2/pi)*d*log(d)*c*b; % seems to be wrong as well see 489
else
